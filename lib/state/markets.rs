...
/// Hash of a market state version for blockchain-like linking
...
/// Immutable market state version that forms a blockchain-like structure
/// 
/// Each market state update creates a new MarketStateVersion that references
/// the previous state, creating an immutable chain of market states as required
/// by the Bitcoin Hivemind specification.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct MarketStateVersion {
    /// Version number (increments with each state change)
    pub version: u64,
    /// Hash of the previous market state (None for genesis state)
    pub previous_state_hash: Option<MarketStateHash>,
    /// Hash of this market state version
    pub state_hash: MarketStateHash,
    /// Block height when this state version was created
    pub created_at_height: u64,
    /// Transaction that caused this state change
    pub transaction_id: Option<[u8; 32]>,
    /// Market state at this version
    pub market_state: MarketState,
    /// LMSR beta parameter at this version
    pub b: f64,
    /// Trading fee at this version
    pub trading_fee: f64,
    /// Share quantities for each valid state at this version
    #[serde(with = "ndarray_1d_serde")]
    pub shares: Array<f64, Ix1>,
    /// Final prices at this version (after resolution)
    #[serde(with = "ndarray_1d_serde")]
    pub final_prices: Array<f64, Ix1>,
    /// Treasury balance at this version
    pub treasury: f64,
...
impl MarketStateVersion {
    /// Create a new market state version
    pub fn new(
        version: u64,
        ...
        let state_data = format!(
            "{}:{}:{}:{}:{}:{}:{}:{}:{}",
            version,
            previous_state_hash.as_ref().map(|h| h.to_string()).unwrap_or_default(),
            created_at_height,
            transaction_id.map(|id| hex::encode(id)).unwrap_or_default(),
            format!("{:?}", market_state),
            b,
            trading_fee,
            shares.to_vec().iter().map(|v| v.to_string()).collect::<Vec<_>>().join(","),
            treasury
        );
    ...
    /// Get the state hash of this version
    pub fn get_state_hash(&self) -> &MarketStateHash {
        &self.state_hash
    }
    
    /// Check if this is a genesis state version (no previous state)
...
impl MarketState {
    /// Validate if a state transition is allowed according to Bitcoin Hivemind rules
    ...
    /// Check if this state allows trading operations
    pub fn allows_trading(&self) -> bool {
        matches!(self, MarketState::Trading)
    }

    /// Check if this state allows voting operations
...
fn parse_single_slot(slot_str: &str) -> Result<SlotId, MarketError> {
    ...
    let slot_id_array: [u8; 3] = slot_bytes.try_into()
        .map_err(|_| MarketError::InvalidDimensions)?;
...
impl ShareAccount {
    ...
    pub fn get_position(&self, market_id: &MarketId, outcome_index: u32) -> f64 {
        self.positions.get(&(market_id.clone(), outcome_index))
            .copied()
            .unwrap_or(0.0)
    }
...
impl BatchedMarketTrade {
    ...
    pub fn calculate_trade_cost(&self) -> Result<f64, MarketError> {
        ...
        let old_cost = lmsr.cost_function(self.market_snapshot.b, &self.market_snapshot.shares.view())
            .map_err(|e| MarketError::DatabaseError(format!("LMSR calculation failed: {:?}", e)))?;
        ...
        let new_cost = lmsr.cost_function(self.market_snapshot.b, &new_shares.view())
            .map_err(|e| MarketError::DatabaseError(format!("LMSR calculation failed: {:?}", e)))?;
...
/// Multidimensional prediction market using LMSR with blockchain-style state management
/// 
/// This structure maintains an immutable chain of market state versions, where each
/// state update creates a new MarketStateVersion that references the previous state.
/// This ensures full auditability and compliance with Bitcoin Hivemind specifications.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct Market {
    /// Unique market identifier
    pub id: MarketId,
    /// Human-readable market title (immutable after creation)
    pub title: String,
    /// Market description (immutable after creation)
    pub description: String,
    /// Search/categorization tags (immutable after creation)
    pub tags: Vec<String>,
    /// Market creator address (immutable after creation)
    pub creator_address: Address,
    /// Decision slot IDs that define market dimensions (immutable after creation)
    pub decision_slots: Vec<SlotId>,
    /// Logical functions defining valid states (immutable after creation)
    pub d_functions: Vec<DFunction>,
    /// Mapping from state index to decision combo (immutable after creation)
    pub state_combos: Vec<Vec<usize>>,
    /// Block height when market was created (immutable after creation)
    pub created_at_height: u64,
    /// Optional expiration height (immutable after creation)
    pub expires_at_height: Option<u64>,
    /// Time until resolution in vote-matrix (immutable after creation)
    pub tau_from_now: u8,
    /// Length of share vector (immutable after creation)
    pub share_vector_length: usize,
    /// Storage fee in sats based on L2 sat/byte rate * N (immutable after creation)
    pub storage_fee_sats: u64,
    /// Size in bytes for storage calculations (immutable after creation)
    pub size: usize,
    /// Blockchain-style history of market states (immutable chain)
    pub state_history: Vec<MarketStateVersion>,
    /// Hash of the current (most recent) state version
    pub current_state_hash: MarketStateHash,
    /// Total volume traded across all outcomes in sats
    pub total_volume_sats: u64,
    /// Per-outcome trading volumes in sats
...
mod ndarray_1d_serde {
    ...
    where
        S: Serializer,
    {
        // Handle the case where array is not contiguous in memory
        match array.as_slice() {
            Some(slice) => slice.serialize(serializer),
            None => {
                // Array is not contiguous, convert to Vec first
...
impl MarketBuilder {
    ...
    /// Calculate and set initial liquidity automatically based on beta parameter
    /// Uses formula: Initial Liquidity = b * ln(Number of States in the Market)
    fn calculate_initial_liquidity(&self) -> u64 {
        ...
        initial_liquidity.ceil() as u64 // Round up to ensure sufficient liquidity
    }

    /// Get the total number of outcomes for this market
    fn get_outcome_count(&self) -> usize {
        if let Some(specs) = &self.dimension_specs {
            ...
            // Independent decision slots: each decision contributes 2 outcomes (binary)
    ...
    pub fn build(
        ...
        let (all_slots, d_functions, state_combos) = if let Some(ref dimension_specs) = self.dimension_specs {
            ...
            for spec in dimension_specs {
                match spec {
                    DimensionSpec::Single(slot_id) => slots.push(slot_id.clone()),
                    DimensionSpec::Categorical(slot_ids) => slots.extend(slot_ids),
                }
            }
            (slots, d_funcs, combos)
        } else if let Some((ref cat_slots, has_residual)) = self.categorical_slots {
...
impl DFunction {
    ...
    pub fn validate_constraint(&self, max_decision_index: usize, decision_slots: &[SlotId]) -> Result<(), MarketError> {
        crate::validation::DFunctionValidator::validate_constraint(self, max_decision_index, decision_slots)
    }
    ...
    pub fn validate_categorical_constraint(
        ...
        crate::validation::DFunctionValidator::validate_categorical_constraint(self, categorical_slots, combo, decision_slots)
    }
    ...
    pub fn validate_dimensional_consistency(
        ...
        crate::validation::DFunctionValidator::validate_dimensional_consistency(d_functions, dimension_specs, decision_slots, all_combos)
    }
    ...
    fn build_balanced_and_tree(mut constraints: Vec<DFunction>) -> DFunction {
        while constraints.len() > 1 {
            ...
            while constraints.len() >= 2 {
                let right = constraints.pop().expect("constraints.len() >= 2 guarantees pop() succeeds");
                let left = constraints.pop().expect("constraints.len() >= 2 guarantees pop() succeeds");
...
pub fn generate_mixed_dimensional(
    ...
    for combo in &state_combos {
        ...
        let d_function = match constraints.len() {
            0 => DFunction::True,
            1 => constraints.into_iter().next().expect("constraints.len() == 1 guarantees next() succeeds"),
            _ => {
    ...
    DFunction::validate_dimensional_consistency(&d_functions, dimension_specs, &all_slots, &state_combos)?;
...
impl Market {
    pub fn new(
        ...
        let initial_treasury = if let Some(capital_sats) = initial_liquidity_sats {
    ...
    pub fn get_current_state(&self) -> &MarketStateVersion {
        self.state_history.last().expect("Market must have at least genesis state")
    }
    
    /// Get market state version by index
    pub fn get_state_version(&self, version: u64) -> Option<&MarketStateVersion> {
    ...
    pub fn create_new_state_version(
        ...
        let market_state = new_market_state.unwrap_or(current_state.market_state);
        ...
        let final_prices = new_final_prices.unwrap_or_else(|| current_state.final_prices.clone());
        ...
        if new_market_state.is_some() && !current_state.market_state.can_transition_to(market_state) {
    ...
    pub fn calc_treasury(&self) -> f64 {
        ...
        lmsr.cost_function(current_state.b, &current_state.shares.view())
            .unwrap_or_else(|_| current_state.b * (current_state.shares.len() as f64).ln())
    }
    ...
    pub fn calculate_prices(&self, shares: &Array<f64, Ix1>) -> Array<f64, Ix1> {
        use crate::math::lmsr::Lmsr;
        let current_state = self.get_current_state();
        // Use LMSR with actual market size instead of hardcoded default (256)
        let lmsr = Lmsr::new(shares.len());
    ...
    pub fn update_shares(
        ...
        let current_state = self.get_current_state();
        let new_treasury = crate::math::lmsr::calculate_cost(&new_shares.to_vec(), current_state.b)
            .map_err(|e| MarketError::DatabaseError(format!("LMSR calculation failed: {:?}", e)))?;
    ...
    pub fn update_trading_volume(&mut self, outcome_index: usize, trade_cost_sats: u64) -> Result<(), MarketError> {
        ...
        self.outcome_volumes_sats[outcome_index] =
            self.outcome_volumes_sats[outcome_index].saturating_add(trade_cost_sats);

        // Update total volume
        self.total_volume_sats = self.total_volume_sats.saturating_add(trade_cost_sats);
    ...
    pub fn query_update_cost(&self, new_shares: Array<f64, Ix1>) -> Result<f64, MarketError> {
        ...
        // Use LMSR with actual market size instead of hardcoded default (256)
        let lmsr = Lmsr::new(current_state.shares.len());
        let current_cost = lmsr.cost_function(current_state.b, &current_state.shares.view())
            .map_err(|e| MarketError::DatabaseError(format!("LMSR calculation failed: {:?}", e)))?;
        let new_cost = lmsr.cost_function(current_state.b, &new_shares.view())
            .map_err(|e| MarketError::DatabaseError(format!("LMSR calculation failed: {:?}", e)))?;
    ...
    pub fn query_amp_b_cost(&self, new_b: f64) -> Result<f64, MarketError> {
        ...
        // Use LMSR with actual market size instead of hardcoded default (256)
        let lmsr = Lmsr::new(current_state.shares.len());
        let new_cost = lmsr.cost_function(new_b, &current_state.shares.view())
            .map_err(|e| MarketError::DatabaseError(format!("LMSR calculation failed: {:?}", e)))?;
    ...
    pub fn calculate_buy_cost(&self, outcome_index: u32, shares_to_buy: f64) -> Result<u64, MarketError> {
...
pub struct MarketsDatabase {
    ...
    share_accounts: DatabaseUnique<SerdeBincode<Address>, SerdeBincode<ShareAccount>>,
}
...
impl MarketsDatabase {
    ...
    pub fn migrate_utxo_to_commitment_model(
        &self,
        txn: &mut RwTxn,
        utxos_db: &DatabaseUnique<SerdeBincode<OutPoint>, SerdeBincode<crate::types::FilledOutput>>,
        height: u64,
    ) -> Result<usize, Error> {
        use std::collections::BTreeMap;
        
        tracing::info!("Starting migration from UTXO-based positions to commitment model at height {}", height);
        
        // Phase 1: Scan all UTXOs for share positions
        let mut address_positions: BTreeMap<Address, BTreeMap<(MarketId, u32), f64>> = BTreeMap::new();
        let mut utxo_count = 0;
        let mut share_utxo_count = 0;
        
        // Iterate through all UTXOs to find share positions
        let utxo_iter = utxos_db.iter(txn)
            .map_err(|e| Error::DatabaseError(format!("UTXO iteration failed: {}", e)))?;
        
        let mut utxo_iter = utxo_iter;
        while let Some(item) = utxo_iter.next().map_err(|e| Error::DatabaseError(format!("UTXO iteration item failed: {}", e)))? {
            let (outpoint, filled_output) = item;
            
            utxo_count += 1;
            
            // Check if this UTXO contains share positions
            if let Some(share_data) = Self::extract_legacy_share_data(&outpoint, &filled_output) {
                share_utxo_count += 1;
                
                // Group by address
                let address = filled_output.address;
                let address_positions_map = address_positions.entry(address).or_default();
                ...
                let current = address_positions_map.get(&key).copied().unwrap_or(0.0);
                address_positions_map.insert(key, current + shares);
                
                tracing::debug!("Found {:.4} shares for market {} outcome {} at address {} in UTXO {:?}",
                           shares, market_id, outcome_index, address, outpoint);
            }
        }
        ...
        tracing::info!("Scanned {} UTXOs, found {} with share positions across {} addresses", 
                   utxo_count, share_utxo_count, address_positions.len());
        ...
        for (address, positions) in address_positions {
            ...
            if self.share_accounts.try_get(txn, &address)
                .map_err(|e| Error::DatabaseError(format!("Share account lookup failed: {}", e)))?
                .is_some() {
                tracing::debug!("Skipping address {} - already has share account (already migrated)", address);
                continue;
            }
            ...
            for ((market_id, outcome_index), shares) in positions.iter() {
                share_account.add_shares(market_id.clone(), *outcome_index, *shares, height);
                total_positions_migrated += 1;
                
                tracing::debug!("Migrated {:.4} shares for market {} outcome {} to address {}",
                           shares, market_id, outcome_index, address);
            }
            ...
            self.share_accounts.put(txn, &address, &share_account)
                .map_err(|e| Error::DatabaseError(format!("Share account creation failed for {}: {}", address, e)))?;
            
            migrated_accounts += 1;
            
            tracing::info!("Successfully migrated {} positions for address {} from UTXO model", positions.len(), address);
        }
        ...
        tracing::info!("Migration completed: {} accounts with {} total positions migrated from UTXO to commitment model", 
                   migrated_accounts, total_positions_migrated);
    ...
    pub fn needs_utxo_migration(
        ...
        while let Some(item) = utxo_iter.next().map_err(|e| Error::DatabaseError(format!("UTXO iteration item failed: {}", e)))? {
            ...
            if Self::extract_legacy_share_data(&outpoint, &filled_output).is_some() {
                return Ok(true);
            }
        }
    ...
    pub fn new(env: &Env, rwtxn: &mut RwTxn) -> Result<Self, Error> {
        ...
        let share_accounts = DatabaseUnique::create(env, rwtxn, "share_accounts")?;
    ...
    pub fn get_markets_batch(&self, txn: &RoTxn, market_ids: &[MarketId]) -> Result<HashMap<MarketId, Market>, Error> {
        ...
        let market_iter = self.markets.iter(txn)
            .map_err(|e| Error::DatabaseError(format!("Market batch iteration failed: {}", e)))?;
        
        let mut market_iter = market_iter;
        while let Some(item) = market_iter.next().map_err(|e| Error::DatabaseError(format!("Market batch iteration item failed: {}", e)))? {
        ...
        tracing::debug!("Batch loaded {}/{} requested markets using optimized iteration", 
                   found_count, market_ids.len());
    ...
    pub fn get_markets_by_expiry(&self, txn: &RoTxn, min_height: Option<u64>, max_height: Option<u64>) -> Result<Vec<Market>, Error> {
        ...
        let expiry_iter = self.expiry_index.iter(txn)
            .map_err(|e| Error::DatabaseError(format!("Expiry index iteration failed: {}", e)))?;
        ...
        while let Some(item) = expiry_iter.next().map_err(|e| Error::DatabaseError(format!("Expiry index iteration item failed: {}", e)))? {
        ...
        tracing::debug!("Expiry range query: checked {} entries, matched {} entries with {} total markets", 
                   entries_checked, entries_matched, matching_market_ids.len());
        ...
        if matching_market_ids.len() > 5 {
            ...
            let markets_map = self.get_markets_batch(txn, &matching_market_ids)?;
        ...
        tracing::debug!("Retrieved {} markets by expiry range [{:?}, {:?}]", 
                   markets.len(), min_height, max_height);
    ...
    pub fn update_voting_markets(
        ...
        for mut market in trading_markets {
            if market.check_voting_period(slots_in_voting, None, 0)? { // TODO: Add transaction_id and height
    ...
    pub fn update_market(&self, txn: &mut RwTxn, market: &Market) -> Result<(), Error> {
        ...
        let old_slots: HashSet<_> = old_market.as_ref()
        ...
        self.markets.put(txn, market.id.as_bytes(), market)
            .map_err(|e| {
                tracing::error!("Failed to update primary market storage for market {}: {}", market.id, e);
                Error::DatabaseError(format!("Primary market update failed: {}", e))
            })?;
        ...
        if let Some(old) = old_market {
            ...
            if old.state() != market.state() {
                self.update_state_index(txn, &market.id, Some(old.state()), Some(market.state()))
                    .map_err(|e| {
                        tracing::error!("Failed to update state index for market {}: {}", market.id, e);
                        Error::DatabaseError(format!("State index update failed: {}", e))
                    })?;
            }
            ...
            if old.expires_at_height != market.expires_at_height {
                self.update_expiry_index(txn, &market.id, old.expires_at_height, market.expires_at_height)
                    .map_err(|e| {
                        tracing::error!("Failed to update expiry index for market {}: {}", market.id, e);
                        Error::DatabaseError(format!("Expiry index update failed: {}", e))
                    })?;
            }
            ...
            for slot_id in old_slots.difference(&new_slots) {
                self.update_slot_index(txn, &market.id, *slot_id, false)
                    .map_err(|e| {
                        tracing::error!("Failed to remove market {} from slot index {}: {}", market.id, hex::encode(slot_id.as_bytes()), e);
                        Error::DatabaseError(format!("Slot index removal failed: {}", e))
                    })?;
            }
            ...
            for slot_id in new_slots.difference(&old_slots) {
                self.update_slot_index(txn, &market.id, *slot_id, true)
                    .map_err(|e| {
                        tracing::error!("Failed to add market {} to slot index {}: {}", market.id, hex::encode(slot_id.as_bytes()), e);
                        Error::DatabaseError(format!("Slot index addition failed: {}", e))
                    })?;
            }
        }
        
        // All operations succeeded - transaction will commit automatically
        tracing::debug!("Successfully updated market {} with all indexes", market.id);
        Ok(())
    }
        ...
        for mut market in resolved_markets {
            ...
            if all_slots_ossified {
                ...
                market.create_new_state_version(
                    ...
                    Some(MarketState::Ossified),
                    None, None, None, None, None
                ).map_err(|e| Error::DatabaseError(format!("Failed to ossify market: {:?}", e)))?;
        ...
        match self.share_accounts.get(rotxn, &mempool_addr) {
            Ok(account) => {
                ...
                if let Some(market) = self.get_market(rotxn, market_id)? {
                    ...
                    for ((account_market_id, outcome_index), &amount) in &account.positions {
    ...
    pub fn put_mempool_shares(
        ...
        for (i, &share_amount) in shares.iter().enumerate() {
            if share_amount != 0.0 {
                account.positions.insert((market_id.clone(), i as u32), share_amount);
            }
        }
    ...
    pub fn process_market_trades_batch(
        ...
        let mut market_updates: HashMap<MarketId, Array<f64, Ix1>> = HashMap::new();
        ...
        tracing::debug!("Validating {} batched market trades", batched_trades.len());
        ...
        for (trade_index, trade) in batched_trades.iter().enumerate() {
            ...
            let market = self.get_market(txn, &market_id)
                .map_err(|e| {
                    tracing::error!("Database error accessing market {} for trade {}: {}", hex::encode(&trade.market_id), trade_index, e);
                    Error::DatabaseError(format!("Market access failed: {}", e))
                })?
                .ok_or_else(|| {
                    tracing::error!("Market {} not found for trade {}", hex::encode(&trade.market_id), trade_index);
            ...
            if market.state() != MarketState::Trading {
                tracing::error!("Market {} is not in trading state for trade {}: {:?}", hex::encode(&trade.market_id), trade_index, market.state());
                return Err(Error::InvalidTransaction {
                    reason: format!("Market {} is not in trading state", hex::encode(&trade.market_id)),
                });
            }
            
            // Validate outcome index is valid for market
            if trade.outcome_index as usize >= market.shares().len() {
                tracing::error!("Invalid outcome index {} for market {} with {} outcomes", trade.outcome_index, hex::encode(&trade.market_id), market.shares().len());
                return Err(Error::InvalidTransaction {
                    reason: format!("Invalid outcome index {} for market", trade.outcome_index),
                });
            }
            
            // Calculate and validate trade cost using market snapshot
            let cost = trade.calculate_trade_cost()
                .map_err(|e| {
                    tracing::error!("Trade cost calculation failed for trade {}: {}", trade_index, e);
            ...
            if cost > trade.max_cost as f64 {
                tracing::error!("Trade {} cost {:.4} exceeds maximum {}", trade_index, cost, trade.max_cost);
                return Err(Error::InvalidTransaction {
                    reason: format!("Trade cost {:.4} exceeds maximum {}", cost, trade.max_cost),
                });
            }
            ...
            let shares_update = market_updates.entry(market_id.clone())
                .or_insert_with(|| Array::zeros(trade.market_snapshot.shares.len()));
        ...
        tracing::debug!("Applying market updates for {} markets", market_updates.len());
        
        for (market_id, share_changes) in market_updates {
            let mut market = self.get_market(txn, &market_id)?
                .ok_or_else(|| {
                    tracing::error!("Market {} disappeared during batch processing", market_id);
                    Error::DatabaseError("Market disappeared during processing".to_string())
                })?;
            
            // Apply accumulated share changes with bounds checking
            let mut new_shares_array = market.shares().clone();
            for (outcome_index, &share_change) in share_changes.iter().enumerate() {
                if share_change != 0.0 {
                    let new_shares = new_shares_array[outcome_index] + share_change;
                    // Validate shares don't go negative (should not happen with proper validation)
                    if new_shares < 0.0 {
                        tracing::error!("Share update would result in negative shares for market {} outcome {}: {} + {} = {}", 
                                  market_id, outcome_index, market.shares()[outcome_index], share_change, new_shares);
            ...
            let new_treasury = market.calc_treasury_with_shares(&new_shares_array);
            ...
            market.create_new_state_version(
                ...
                Some(new_treasury),
            ).map_err(|e| Error::DatabaseError(format!("Failed to create new market state: {:?}", e)))?;
            
            // Update market in database with full error context
            self.update_market(txn, &market)
                .map_err(|e| {
                    tracing::error!("Failed to update market {} during batch processing: {}", market_id, e);
                    e // Propagate the detailed error from update_market
                })?;
            
            tracing::debug!("Successfully updated market {} with new treasury: {:.4}", market_id, market.treasury());
        }
        
        // Phase 3: Update share accounts atomically with market updates
        tracing::debug!("Updating share accounts for {} trades", batched_trades.len());
        ...
        for (trade_index, trade) in batched_trades.iter().enumerate() {
            self.add_shares_to_account(
                ...
                tracing::error!("Failed to update share account for trade {}: {}", trade_index, e);
                Error::DatabaseError(format!("Share account update failed for trade {}: {}", trade_index, e))
            })?;
        }
        
        tracing::info!("Successfully processed {} batched market trades with total cost: {:.4}", 
                  batched_trades.len(), trade_costs.iter().sum::<f64>());
    ...
    pub fn add_shares_to_account(
        ...
        let mut account = self.share_accounts.try_get(txn, address)?
    ...
    pub fn remove_shares_from_account(
        ...
        account.remove_shares(market_id, outcome_index, shares, height)
    ...
    pub fn get_user_share_positions(
        ...
        if let Some(account) = self.get_user_share_account(txn, address)? {
            Ok(account.positions.into_iter()
                .map(|((market_id, outcome_index), shares)| (market_id, outcome_index, shares))
                .collect())
        } else {
            Ok(Vec::new())
        }
...
mod tests {
    ...
    fn test_dfunction_constraint_validation() {
        ...
        assert!(invalid_func.validate_constraint(2, &decision_slots).is_err());
        
        // Test valid equals constraint
        let valid_equals = DFunction::Equals(Box::new(DFunction::Decision(0)), 1);
        assert!(valid_equals.validate_constraint(2, &decision_slots).is_ok());
        
        // Test invalid equals constraint (bad value)
        let invalid_equals = DFunction::Equals(Box::new(DFunction::Decision(0)), 5);
        assert!(invalid_equals.validate_constraint(2, &decision_slots).is_err());
        ...
        assert!(invalid_nested.validate_constraint(2, &decision_slots).is_err());
    }
    ...
    fn test_categorical_constraint_validation() {
        ...
        assert!(df.validate_categorical_constraint(&categorical_slots, &valid_combo, &decision_slots).unwrap());
        
        // Test residual case (all false)
        let residual_combo = vec![0, 0, 0];
        assert!(df.validate_categorical_constraint(&categorical_slots, &residual_combo, &decision_slots).unwrap());
        
        // Test invalid constraint (multiple true)
        let invalid_combo = vec![1, 1, 0];
        assert!(!df.validate_categorical_constraint(&categorical_slots, &invalid_combo, &decision_slots).unwrap());
        
        // Test out of bounds
        let oob_slots = vec![0, 1, 5]; // Index 5 doesn't exist
        assert!(df.validate_categorical_constraint(&oob_slots, &valid_combo, &decision_slots).is_err());
    }
    ...
    fn test_lmsr_initialization_spec_compliance() {
        ...
        println!("  β = {}, n = {}, L = {}", beta, n_outcomes, target_liquidity);
        ...
        let calculated_treasury = beta * shares.mapv(|x| (x / beta).exp()).sum().ln();
        ...
        assert!((calculated_treasury - target_liquidity).abs() < 1e-10, 
                "Treasury {:.6} should equal target liquidity {:.6}", 
                calculated_treasury, target_liquidity);
        
        // Verify initial prices are uniform
        let exp_shares: Array<f64, ndarray::Ix1> = shares.mapv(|x| (x / beta).exp());
        ...
        for (i, &price) in prices.iter().enumerate() {
            ...
            println!("  Price[{}] = {:.6}, expected = {:.6}", i, price, expected_price);
            assert!((price - expected_price).abs() < 1e-10,
                    "Price[{}] should be {:.6} but was {:.6}", i, expected_price, price);
        }
        ...
        for &price in prices.iter() {
            let expected_price = 1.0 / n_outcomes;
            assert!((price - expected_price).abs() < 1e-10,
                    "All prices should be uniform at {:.6}", expected_price);
        }
        ...
        println!("  β = {}, n = {}, min L = {:.6}", beta, n_outcomes, min_liquidity);
        
        // At minimum liquidity, shares should be 0
        let expected_shares: f64 = min_liquidity - min_liquidity; // Should be 0
        assert!(expected_shares.abs() < 1e-10, "Shares should be 0 at minimum liquidity");
        
        // Treasury with zero shares should equal β × ln(n)
        let shares = Array::zeros(4);
        let calculated_treasury = beta * shares.mapv(|x: f64| (x / beta).exp()).sum().ln();
        
        println!("  Treasury with zero shares = {:.6}", calculated_treasury);
        assert!((calculated_treasury - min_liquidity).abs() < 1e-10,
    ...
    fn test_liquidity_validation() {
        ...
        assert!(expected_shares > 0.0 && expected_shares.is_finite(),
