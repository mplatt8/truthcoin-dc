...
use truthcoin_dc::{
    math::voting::{
        MatrixUtils, ReputationVector, SparseVoteMatrix, VoteAggregator,
    },
    state::{
        State,
        slots::SlotId,
        voting::{
            VotingSystem,
            types::{
                Vote, VoteValue, VoterId, VotingPeriod, VotingPeriodId,
                VotingPeriodStatus,
            },
        },
    },
...
impl BenchEnv {
    fn new() -> Self {
        ...
        let env = Env::new(&temp_dir, 1024 * 1024 * 500)
            .expect("Failed to create environment");
        let mut rwtxn = env.rw_txn().expect("Failed to create transaction");
        let state =
            State::new(&env, &mut rwtxn).expect("Failed to create state");
    ...
    group.bench_function("period_status_updates", |b| {
        ...
        b.iter(|| {
            ...
            let _updates = env
                .state
                .voting_system()
                .get_periods_needing_update(&rotxn, 1500)
                .unwrap();
            rotxn.commit().unwrap();
        });
    });
        ...
        group.bench_with_input(
            ...
            |b, &num_votes| {
                ...
                // Setup period
                {
                    ...
                    env.state
                        .voting_system()
                        .activate_voting_period(&mut rwtxn, period_id, 1000)
                        .unwrap();
                    rwtxn.commit().unwrap();
                }
    ...
    group.bench_function("get_all_votes_for_period", |b| {
        b.iter(|| {
            ...
            let _votes = env
                .state
                .voting_system()
                .get_votes_for_period(&rotxn, period_id)
                .unwrap();
            rotxn.commit().unwrap();
        });
    });
    ...
    group.bench_function("generate_vote_matrix", |b| {
        b.iter(|| {
            ...
            let _matrix = env
                .state
                .voting_system()
                .get_vote_matrix(&rotxn, period_id)
                .unwrap();
            rotxn.commit().unwrap();
        });
    });
    ...
    group.bench_function("calculate_participation_stats", |b| {
        b.iter(|| {
            ...
            let _stats = env
                .state
                .voting_system()
                .get_participation_stats(&rotxn, period_id)
                .unwrap();
            rotxn.commit().unwrap();
        });
    });
    ...
    group.bench_function("get_reputation_weights", |b| {
        b.iter(|| {
            ...
            let _weights = env
                .state
                .voting_system()
                .get_reputation_weights(&rotxn, period_id)
                .unwrap();
            rotxn.commit().unwrap();
        });
    });
...
fn bench_sparse_matrix_operations(c: &mut Criterion) {
    ...
    for &(num_voters, num_decisions) in
        [(100, 50), (500, 100), (1000, 200), (2000, 500)].iter()
    {
        let voters = create_test_voter_ids(num_voters);
        let decisions = create_test_decision_ids(num_decisions);
        ...
        group.bench_with_input(
            BenchmarkId::new(
                "matrix_creation",
                format!("{}x{}", num_voters, num_decisions),
            ),
        ...
        group.bench_with_input(
            ...
            |b, (voters, decisions)| {
                b.iter(|| {
                    let mut matrix = SparseVoteMatrix::new(
                        voters.clone(),
                        decisions.clone(),
                    );
...
fn bench_vote_aggregation(c: &mut Criterion) {
    ...
    for num_votes in [100, 500, 1000, 5000].iter() {
        ...
        group.bench_with_input(
            ...
            &votes,
            |b, votes| {
                b.iter(|| VoteAggregator::simple_majority(votes));
            },
        );
        ...
        group.bench_with_input(
            ...
            &(&votes, &reputation),
            |b, (votes, reputation)| {
                b.iter(|| VoteAggregator::weighted_average(votes, reputation));
            },
        );
        ...
        group.bench_with_input(
            ...
            &votes,
            |b, votes| {
                b.iter(|| VoteAggregator::median_vote(votes));
            },
        );
        ...
        group.bench_with_input(
            ...
            &votes,
            |b, votes| {
                b.iter(|| VoteAggregator::calculate_confidence(votes, 0.7));
            },
        );
    }
...
fn bench_matrix_utils(c: &mut Criterion) {
    ...
    for i in 0..num_voters {
        for j in 0..num_decisions {
            if (i + j) % 4 == 0 {
                ...
                let vote_value =
                    (i + j) as f64 / (num_voters + num_decisions) as f64;
                ...
                if let (Some(&actual_voter), Some(&actual_decision)) = (
                    ...
                    matrix
                        .get_decisions()
                        .get(j % matrix.get_decisions().len()),
                ) {
                    matrix
                        .set_vote(actual_voter, actual_decision, vote_value)
                        .unwrap();
                }
            }
        }
    ...
    group.bench_function("participation_rates", |b| {
        b.iter(|| MatrixUtils::calculate_participation_rates(&matrix));
    });

    group.bench_function("voter_activity", |b| {
        b.iter(|| MatrixUtils::calculate_voter_activity(&matrix));
    });

    group.bench_function("outlier_detection", |b| {
        b.iter(|| MatrixUtils::find_outlier_voters(&matrix, 2.0).unwrap());
    });
...
fn bench_decision_resolution(c: &mut Criterion) {
    ...
    for num_decisions in [10, 50, 100, 500].iter() {
        ...
        // Setup period with votes
        {
            ...
            env.state
                .voting_system()
                .close_voting_period(&mut rwtxn, period_id, 2000)
                .unwrap();
            rwtxn.commit().unwrap();
        }
...
fn bench_system_statistics(c: &mut Criterion) {
    ...
    for period_num in 1..=5 {
        ...
        if period_num <= 3 {
            ...
            for (i, voter) in voters.iter().enumerate() {
                if i % 5 == 0 {
                    // 20% voter participation
                    for (j, decision) in decisions.iter().enumerate() {
                        if j % 3 == 0 {
                            // 33% decision participation
                            let vote_value = if (i + j) % 2 == 0 {
                                VoteValue::Binary(true)
                            } else {
    ...
    group.bench_function("get_system_stats", |b| {
        b.iter(|| {
            let rotxn = env.state.ro_txn().unwrap();
            let _stats =
                env.state.voting_system().get_system_stats(&rotxn).unwrap();
            rotxn.commit().unwrap();
        });
    });
...
fn bench_memory_usage(c: &mut Criterion) {
    ...
    for &(voters, decisions, density) in
        [(1000, 500, 0.1), (2000, 1000, 0.05), (5000, 2000, 0.02)].iter()
    {
        group.bench_with_input(
            BenchmarkId::new(
